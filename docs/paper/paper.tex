...
One alternative to hide the \emph{context} concept from application developers could be to introduce a \textit{build} step that detects the variables captured by p-j-s callbacks and transforms the code. By doing this, theyg wouldn't need to do anything out of the ordinary with regards to closures and their environment when working with p-j-s. However, this approach would require transforming the original code, and would be complex and time consuming to implement.
