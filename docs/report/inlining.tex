...
One of the things found while comparing the performance of the parallel and non-parallel implementations of \code{map} is that our parallel implementation performed a lot worse than what the one expected when considering the size of the \ttarray{} and the \code{mapper} function.
...
\note{The serial works with a \code{Uint8ClampedArray} but accesses three elements at the same time. The parallel implementation uses a \code{Uint32Array} and works on one pixel (A,R,G,B) at a time.}
...
Taking the experience from previous experiments into account we considered that the performance of the two implementation with a \code{Uint32Array} of about \(8E7\) (eight million) elements should be similar, even favoring the parallel one. The initial measurements showed that the linear implementation fared approximately four times better than the parallel one.
...
\subsubsection{Serial implementation generated code}
Figures \ref{fig:parallel-clamp} through \ref{fig:parallel-noise} are related to the serial implementation. As it can be seen from them, only \code{Math.random} is being inlined.
...
\bmcode{http://jsperf.com/pjs-map-inlining}{../../spikes/3.1.3/pjsMapInlining/pjsMapInlining.js}

The results for it are displayed in Figure~\ref{fig:pjs-map-inlining}.
\img{Comparing string encoding/decoding approaches}{../../spikes/3.1.3/pjsMapInlining/pjsMapInlining}{fig:pjs-map-inlining}
